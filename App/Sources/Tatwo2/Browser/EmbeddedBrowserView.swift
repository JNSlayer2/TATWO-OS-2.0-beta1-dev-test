// 照搬自 Apps/TatwoUltraworkMac/Sources/TatwoUltraworkMac/EmbeddedBrowserView.swift；改動 3 行（原因：加入照搬來源標記；移除薄殼不可依賴的 TatwoCEFBridge、TatwoUltraworkCore import）
import AppKit
import SwiftUI
import WebKit
import Darwin
...
struct EmbeddedBrowserView: View {
    ...
    /// 只有右側面板那一顆瀏覽器接受引擎（瀏覽器橋）的網址請求；inspector 那顆不接，免得兩顆搶同一個請求
    private let agentControllable: Bool
    ...
    @State private var refreshedProfileAccessRevision: Int?
    ...
    @State private var agentNavigations: [TatwoBrowserLaneID: BrowserAgentNavigation] = [:]
    ...
    init(
        ...
        if isUIFixture {
        ...
        } else if let saved = model.browserLanes(for: sessionID), !saved.laneState.lanes.isEmpty {
            // 2.0：這條討論串上次留下的分頁組
            lanes = saved.laneState
            var urls: [TatwoBrowserLaneID: URL] = [:]
            for (key, value) in saved.laneURLs { urls[TatwoBrowserLaneID(rawValue: key)] = value }
            _laneURLs = State(initialValue: urls)
            // Initial construction must restore the selected page too.
            // onChange(sessionID) only handles later switches.
            if let selected = lanes.selectedLaneID, let url = urls[selected] {
                _addressText = State(initialValue: url.absoluteString)
                _currentURLString = State(initialValue: url.absoluteString)
                _isBrowserRuntimeVisible = State(initialValue: true)
            }
        } else {
            let firstLaneID = TatwoBrowserLaneID(
            ...
            lanes = TatwoBrowserLaneReducer.reduce(
                ...
                action: .open(
                    ...
                    title: "新分頁"),
                now: Date())
            if EmbeddedBrowserTestBlankLanePolicy.isEnabled() {
                // DEBUG-only 驗收鉤子：第一個分頁直接掛 about:blank（不經 resolver），renderer 才會真的起來
                _laneURLs = State(initialValue: [firstLaneID: EmbeddedBrowserTestBlankLanePolicy.blankURL])
                _isBrowserRuntimeVisible = State(initialValue: true)
            } else {
                _laneURLs = State(initialValue: [:])
            }
    ...
    private var selectedLaneURL: URL? {
        ...
        return agentNavigations[selectedLaneID]?.url ?? laneURLs[selectedLaneID]
    }

    private var runtimeAccessRequested: Bool {
        isBrowserRuntimeVisible
            || (browserEngine == .chromiumCEF && !laneURLs.isEmpty)
    }
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            if browserEngine == .chromiumCEF {
                // Keep visited native tabs mounted behind the blank start page.
                // The host hides inactive NSViews; saved URLs are not renderers.
                ZStack {
                    browserSurface
                        .id(EmbeddedChromiumBrowserMountIdentity(profile: browserProfile))
                        .allowsHitTesting(isBrowserRuntimeVisible)
                        .accessibilityHidden(!isBrowserRuntimeVisible)
                    if !isBrowserRuntimeVisible { browserStartPage }
                }
                .frame(maxWidth: .infinity, maxHeight: .infinity)
            } else if isBrowserRuntimeVisible {
                browserSurface
                    .id(
        ...
        // 2.0 水電接點：引擎經瀏覽器橋要求打開某個網址 → 走跟使用者按 Enter 一樣的 navigate(to:)
        .onAppear {
            // 面板剛開時 .task 會先做初始化（可能把狀態重設回起始頁），延後半秒再吃引擎的網址請求
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) { consumeBrowserAgentRequest() }
        }
        .onChange(of: model.requestedBrowserAgentURL) { _, _ in consumeBrowserAgentRequest() }
        // 2.0：分頁組跟著討論串走（存／切／被設定頁關閉）
        .onChange(of: laneState) { _, _ in model.storeBrowserLanes(laneState, laneURLs: laneURLs, for: sessionID) }
        .onChange(of: laneURLs) { _, _ in model.storeBrowserLanes(laneState, laneURLs: laneURLs, for: sessionID) }
        .onChange(of: sessionID) { old, new in
            model.storeBrowserLanes(laneState, laneURLs: laneURLs, for: old)
            switchLanes(to: model.browserLanes(for: new))
        }
        .onChange(of: model.browserLanesBySession[sessionID ?? ""] == nil) { _, closed in
            if closed, !laneURLs.isEmpty { switchLanes(to: nil) }
        }
        .onChange(of: isBrowserRuntimeVisible) { _, v in NSLog("tatwo2 browser-agent: runtimeVisible -> %d", v ? 1 : 0) }
        .onChange(of: profileAccessState) { _, st in NSLog("tatwo2 browser-agent: profileAccess -> %@", String(describing: st)) }
        .onDisappear {
            model.clearBrowserAgentActivePage(sessionID: sessionID)
        }
        .task(
            id:
                "\(browserEngine.rawValue):\(browserProfile.registryKey.uuidString):\(profileAccessRevision):\(runtimeAccessRequested)"
        ) {
            guard
                runtimeAccessRequested,
                EmbeddedBrowserUIFixturePolicy.allowsRealActions(
                    isFixture: isUIFixture)
            else {
                return
            }
            // Closing the last page and opening another must not tear down a
            // ready profile. Explicit retry/maintenance revisions still refresh.
            if browserEngine == .chromiumCEF,
               profileAccessState.isReady(for: browserProfile.registryKey),
               refreshedProfileAccessRevision == profileAccessRevision {
                return
            }
            refreshedProfileAccessRevision = profileAccessRevision
    ...
    private var browserRuntime: some View {
        ...
        if EmbeddedBrowserRuntimeMountPolicy.allowsMount(
            ...
            profileKey: profileKey)
        {
            ...
            case .chromiumCEF:
                EmbeddedChromiumBrowserView(
                    ...
                    initialURL: isBrowserRuntimeVisible ? selectedLaneURL : nil,
                    initialAgentNavigation: laneState.selectedLaneID.flatMap { agentNavigations[$0] },
                    openTabIDs: Set(laneState.lanes.map { $0.id.rawValue }),
                    command: command,
                    isGeometryDragInProgress: isPanelResizing,
                    onNavigationStateChange: { tabID, state in
                        guard browserProfile.registryKey == profileKey else { return }
                        let laneID = TatwoBrowserLaneID(rawValue: tabID)
                        guard laneState.lanes.contains(where: { $0.id == laneID }) else { return }
                        retainNavigationURL(state, for: laneID)
                        // Background callbacks belong to their original tab,
                        // never whichever tab happens to be selected now.
                        guard laneState.selectedLaneID == laneID,
                              isBrowserRuntimeVisible else { return }
    ...
    private func clearActivePageVisibleState(preserveAddressDraft: Bool = false) {
        model.clearBrowserAgentActivePage(sessionID: sessionID)
        addressText = EmbeddedBrowserAddressPresentation.text(
            draft: addressText, navigationURL: nil,
            isEditing: preserveAddressDraft && (addressFieldFocused || startPageFieldFocused))
        if !preserveAddressDraft {
            addressFieldFocused = false
            startPageFieldFocused = false
        }
        currentURLString = ""
        canGoBack = false
        canGoForward = false
        ...
        VStack(alignment: .leading, spacing: 4) {
            HStack(spacing: 6) {
                browserNavigationButton(
                    "chevron.left", label: "上一頁",
                    enabled: canGoBack, action: .goBack)
                browserNavigationButton(
                    "chevron.right", label: "下一頁",
                    enabled: canGoForward, action: .goForward)
                browserNavigationButton(
                    "arrow.clockwise", label: "重新載入",
                    enabled: isBrowserRuntimeVisible && !currentURLString.isEmpty,
                    action: .reload)
            ...
            // Show actionable failures only, not a permanent runtime explanation.
            if let validationMessage, !validationMessage.isEmpty {
                Text(validationMessage)
                    ...
                    .accessibilityIdentifier("browser-address-error")
            }
        }
    ...
    private func browserNavigationButton(
        _ symbol: String, label: String, enabled: Bool,
        action: EmbeddedBrowserCommand.Action
    ) -> some View {
        Button {
            addressFieldFocused = false
            startPageFieldFocused = false
        ...
        .buttonStyle(.plain)
        .disabled(!enabled || !profileAccessState.isReady(for: browserProfile.registryKey))
        .help(label)
    ...
    private func selectLane(_ id: TatwoBrowserLaneID) {
        ...
        // This is a user tab selection, not a delayed agent navigation.
        agentNavigations[id] = nil
        addressFieldFocused = false
        startPageFieldFocused = false
    ...
    private func closeLane(_ id: TatwoBrowserLaneID) {
        ...
        agentNavigations[selectedLaneID] = nil
    ...
    /// 換到另一條討論串的分頁組（nil＝全新的新分頁）
    private func switchLanes(to saved: BrowserLaneSnapshot?) {
        command = nil
        agentNavigations = [:]
        validationMessage = nil
        navigationState = .blank
        clearActivePageVisibleState()
        if let saved, !saved.laneState.lanes.isEmpty {
            var urls: [TatwoBrowserLaneID: URL] = [:]
            for (key, value) in saved.laneURLs { urls[TatwoBrowserLaneID(rawValue: key)] = value }
            laneState = saved.laneState
            laneURLs = urls
            if let id = laneState.selectedLaneID, let url = urls[id] {
                addressText = url.absoluteString
                currentURLString = url.absoluteString
                isBrowserRuntimeVisible = true
            } else {
                addressText = ""; currentURLString = ""; isBrowserRuntimeVisible = false
            }
        } else {
            laneState = TatwoBrowserLaneReducer.reduce(
                state: TatwoBrowserLaneState(),
                action: .open(id: TatwoBrowserLaneID(rawValue: "browser-\(UUID().uuidString.lowercased())"), binding: .unboundReadOnly, title: "新分頁"),
                now: Date())
            laneURLs = [:]
            addressText = ""; currentURLString = ""; isBrowserRuntimeVisible = false
        }
    ...
    private func consumeBrowserAgentRequest() {
        guard agentControllable else { return }
        // The delayed onAppear/onChange must consume its original request,
        // not navigate the newly selected chat after a switch or local Stop.
        guard let navigation = model.consumeBrowserAgentNavigation(for: sessionID) else { return }
        if laneState.selectedLaneID == nil { openNewTab() }
        navigate(to: navigation.url, agentNavigation: navigation)
    }

    private func navigate(to url: URL, agentNavigation: BrowserAgentNavigation? = nil) {
        guard let selectedLaneID = laneState.selectedLaneID else { return }
        addressFieldFocused = false
        startPageFieldFocused = false
        agentNavigations[selectedLaneID] = agentNavigation
        // Unsent agent URLs must not enter saved tab state and reappear as
        // ordinary, ungranted navigation after Stop or a chat switch.
        if agentNavigation == nil { laneURLs[selectedLaneID] = url }
        validationMessage = nil
        addressText = url.absoluteString
        currentURLString = url.absoluteString
        ...
        if isBrowserRuntimeVisible,
           profileAccessState.isReady(for: browserProfile.registryKey)
        {
            issue(.load(url), agentNavigation: agentNavigation)
        } else {
            command = nil
            navigationState = .blank
            isBrowserRuntimeVisible = true
    ...
    private func issue(_ action: EmbeddedBrowserCommand.Action,
                       agentNavigation: BrowserAgentNavigation? = nil) {
        guard EmbeddedBrowserUIFixturePolicy.allowsRealActions(
            isFixture: isUIFixture)
        else {
            return
        ...
        guard EmbeddedBrowserCommandDispatchPolicy.dispatch(
            ...
            state: profileAccessState,
            profileKey: browserProfile.registryKey,
            sink: { command = EmbeddedBrowserCommand(action: $0, agentNavigation: agentNavigation) })
        else {
            validationMessage =
                "瀏覽器尚未就緒，請稍後再試。"
            return
    ...
    private func applyNavigationState(
        ...
        {
            clearActivePageVisibleState(preserveAddressDraft: true)
        } else if let urlString = state.urlString {
            addressText = EmbeddedBrowserAddressPresentation.text(
                draft: addressText, navigationURL: urlString,
                isEditing: addressFieldFocused || startPageFieldFocused)
            currentURLString = urlString
            if let selectedLaneID = laneState.selectedLaneID {
                retainNavigationURL(state, for: selectedLaneID)
            }
        }
    }

    private func retainNavigationURL(_ state: EmbeddedBrowserNavigationState,
                                     for laneID: TatwoBrowserLaneID) {
        if let url = Self.committedURLForPersistence(state) { laneURLs[laneID] = url }
    }
