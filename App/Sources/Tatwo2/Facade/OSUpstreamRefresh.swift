...
enum OSUpstreamRefresh {
    ...
    static func applyOnLaunch(
        ...
        do {
            ...
            let installed = fm.fileExists(atPath: marker.path)
            ...
            if installed == nil, current == digest {
                try Data((digest + "\n").utf8).write(to: marker, options: .atomic)
                return .unchanged
            }
            guard let installed, current == installed else {
                // Hashes detect a changed bundle, not chronological version ordering.
                if digest != current && digest != installed {
            ...
            guard digest != installed else { return .unchanged }
