...
5. 每個落地的改動，使用者當天就能用到。
6. 事故寫進 `經驗.md`，不寫進程式碼。
7. 發行通道（2026-09-13）：公開版 `vX.Y.Z` 三段、私人版 `vX.Y.Z.NNN` 四段（NNN 從 001 遞增，是下一個公開版 `X.Y.(Z+1)` 的候選）；排序 `2.0.5 < 2.0.5.001 < 2.0.5.002 < 2.0.6`。每個改動先發私人版、內部裝置 soak；使用者授權後由 `scripts/promote-release.sh --i-authorize` 把最後一個候選同 commit 重打包成 `X.Y.(Z+1)` 公私同版，該列車即結束、舊候選標 pre-release，下一列車從 `X.Y.(Z+1).001` 重新起算。沒授權就不得進公開倉。
